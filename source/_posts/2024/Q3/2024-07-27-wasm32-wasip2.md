...
WASI has released two milestone versions, [**0.1**](https://github.com/WebAssembly/WASI/blob/main/legacy/preview1/docs.md) and [**0.2**](https://github.com/WebAssembly/WASI/tree/main/wasip2). They correspond to Rust targets [`wasm32-wasip1`](https://doc.rust-lang.org/nightly/rustc/platform-support/wasm32-wasip1.html) and [`wasm32-wasip2`](https://doc.rust-lang.org/nightly/rustc/platform-support/wasm32-wasip2.html) respectively.
...
- ✓ `WASI SDK 23.0` or [later](https://github.com/WebAssembly/wasi-sdk/releases)
- ✓ `wasmtime 23.0.1` or [later](https://github.com/bytecodealliance/wasmtime/releases)
- ✓ `jco 1.3.1` or [later](https://github.com/bytecodealliance/jco/releases) (**wasm32-wasip2** only)
- ⍻ `WasmEdge 0.14.0` or [later](https://github.com/WasmEdge/WasmEdge/releases) (wasm32-wasip1 only)
- ⍻ `wasmer 4.3.5` or [later](https://github.com/wasmerio/wasmer/releases) (wasm32-wasip1 only)
- ⍻ `wasmi 0.36.0` or [later](https://github.com/wasmi-labs/wasmi/releases) (No binary packages, requires `rust 1.77.0` or newer to compile) (wasm32-wasip1 only)
- ⍻ `wazero 1.7.3` or [later](https://github.com/tetratelabs/wazero/releases) (wasm32-wasip1 only)
- ⍻ `WebAssembly Micro Runtime 2.1.1` or [later](https://github.com/bytecodealliance/wasm-micro-runtime/releases) (wasm32-wasip1 only)
...
All WASI implementations except `jco` used in this article support the target `wasm32-wasip1`:
...
[`jco`](https://github.com/bytecodealliance/jco) **only** support target `wasm32-wasip2`:

```bash
# npm install @bytecodealliance/jco

# jco run hello-p2.wasm
Hello, world!
```
...
```bash
# rustc -v -C strip=symbols --target wasm32-wasip1 -o hello-p1.wasm hello.rs
...
```bash
# rustc -v -C strip=symbols --target wasm32-wasip2 -o hello-p2.wasm hello.rs

# file hello-p2.wasm
hello-p2.wasm: WebAssembly (wasm) binary module version 0x1000d
```

Currently only [`wasmtime`](https://github.com/bytecodealliance/wasmtime/) & [`jco`](https://github.com/bytecodealliance/jco) support the target `wasm32-wasip2`:
